...
class UnitreeEnv(gym.Env):
    def __init__(self, model_path, render_mode=None, frame_skip=4):
        ...
        self.viewer = None
        self.renderer = None # <--- Added for RGB rendering
        self.camera = None   # <--- Added for RGB camera tracking
    ...
    def reset(self, seed=None, options=None):
        ...
        # Initialize Viewer if "human" mode
    ...
    def render(self):
        """
        New Render Method supporting RGB Arrays for side-by-side comparison
        """
        if self.render_mode == "rgb_array":
            # Initialize Renderer if needed
            if self.renderer is None:
                self.renderer = mujoco.Renderer(self.model, height=480, width=640)
                # Initialize a tracking camera
            ...
            # Just verify viewer exists (handled in step, but good safety)
            pass
